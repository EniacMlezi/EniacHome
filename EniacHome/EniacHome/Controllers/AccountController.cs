...
namespace EniacHome.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(List<LoginViewUserModel> model, string UserIndex, string returnUrl)
        {
            ...
            if (user != null)
            {
                if (!await UserManager.IsEmailConfirmedAsync(user.Id) || !user.AccountConfirmed)
                {
                    ModelState.AddModelError("", "You must have a confirmed account and/or email to log in.");
                    return View(model);
                }
            }
            ...
            {
                ModelState.AddModelError("failedLogin", "Invalid username/password combination.");
                ViewBag.ReturnUrl = returnUrl;
                return View(model);
            }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {   
                    var callbackUrl = Url.Action("ConfirmAccount", "User", new { area = "Admin", userId = user.Id }, protocol: Request.Url.Scheme);
                    ...
                    foreach (var User in UserManager.Users.ToList())
                    {
                        ...
                        {
                           await UserManager.SendEmailAsync(User.Id, "confirm Account", String.Format("Dear Moderator,<br /><br />The following user has registered to your house:<br />{0}<br />{1}<br />{2}<br />{3}<br /><br />Confirm this account by clicking <a href=\"{4}\">here</a>",
                                    user.FullName,
                                    user.Email,
                                    user.Address,
                                    user.Country,
                    ...
                    ViewBag.Message = "An administrator has been notified. You will receive an email soon.";
